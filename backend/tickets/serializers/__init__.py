...
Main serializers are in tickets.serializers (parent file)
This package contains additional serializers for SLA timers
"""
...
if _parent_serializers.exists():
    spec = importlib.util.spec_from_file_location("tickets.serializers_main", _parent_serializers)
    if spec and spec.loader:
        serializers_main = importlib.util.module_from_spec(spec)
        serializers_main.__package__ = 'tickets'
...
try:
    from .sla_timers import (
        TicketSLASerializer, TicketSLACreateSerializer,
        TicketSLAMarkFirstResponseSerializer, TicketSLAMarkResolvedSerializer
    )
except ImportError:
    TicketSLASerializer = None
    TicketSLACreateSerializer = None
    TicketSLAMarkFirstResponseSerializer = None
    TicketSLAMarkResolvedSerializer = None
