...
class PayoutSelectors:
    ...
    def get_batch_for_window(window: PaymentWindow) -> PayoutBatch | None:
        try:
            return (
                ...
                .get(payment_window=window)
            )
        except PayoutBatch.DoesNotExist:
            return None

    @staticmethod
    def get_record_for_writer(
        batch: PayoutBatch,
        writer,
    ) -> PayoutRecord | None:
    ...
    @staticmethod
    def get_held_records_for_site(website):
        """All held payout records site-wide — admin / support priority queue."""
        return (
            PayoutRecord.objects
            .filter(
                batch__payment_window__website=website,
                status=PayoutRecordStatus.HELD,
            )
            .select_related("writer", "batch__payment_window")
            .order_by("-batch__created_at")
        )

    @staticmethod
    def get_writer_payout_history(writer):
        """All payout records for one writer across all windows."""
        return (
            PayoutRecord.objects
    ...
    def get_batch_record_counts(batch: PayoutBatch) -> dict:
        qs = PayoutRecord.objects.filter(batch=batch).values(
            "status"
