"""
Management command to seed writer compensation with sample data.
Creates compensation entries with various statuses, linked to orders and special orders.
"""
...
class Command(BaseCommand):
    help = 'Seed writer compensation records with sample data'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear existing writer compensation before seeding',
        )
        parser.add_argument(
            '--count',
            type=int,
            default=25,
            help='Number of compensation entries to create per website (default: 25)',
        )
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            if clear:
                self.stdout.write('Clearing existing writer compensation...')
            ...
            for website in websites:
                ...
                # Get orders in various states for different compensation statuses.
                # Completed orders -> paid compensation
