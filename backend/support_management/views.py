...
class SupportProfileViewSet(viewsets.ModelViewSet):
    queryset = SupportProfile.objects.order_by("id")
    ...
    def get_queryset(self):
        ...
        if self.request.user.role == "support":
            return SupportProfile.objects.filter(
...
class SupportNotificationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SupportNotification.objects.filter(
            support_staff=self.request.user.support_profile,
...
class EscalationLogViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        if request.user.role in ["admin", "superadmin"]:
            ...
            return Response({"message": "Escalation approved."},
...
class PaymentIssueLogViewSet(viewsets.ModelViewSet):
    ...
    def escalate_issue(self, request, pk=None):
        ...
        return Response({"message": "Payment issue escalated."},
                        status=status.HTTP_200_OK)
...
class SupportDashboardViewSet(viewsets.ModelViewSet):
    ...
    def refresh_dashboard(self, request):
        ...
        return Response(
            {"message": "Support dashboards refreshed."}, status=status.HTTP_200_OK)
    ...
    def dashboard_tickets(self, request):
        ...
        return Response(
            {
                'tickets': serializer.data, 'count': len(
                    serializer.data), 'total_open': Ticket.objects.filter(
                    status__in=[
                        'open', 'in_progress']).count(), 'total_assigned_to_me': Ticket.objects.filter(
                        assigned_to=request.user).count() if request.user.role == "support" else None, })
    ...
    def dashboard_queue(self, request):
        ...
        overdue = Ticket.objects.filter(
            status__in=[
                'open',
                'in_progress'],
            created_at__lt=overdue_threshold).select_related(
            'created_by',
            'assigned_to',
            'website').order_by('-created_at')
    ...
    def dashboard_workload(self, request):
        ...
        sla_compliance_rate = (
            sla_compliant /
            resolved_tickets *
            100) if resolved_tickets > 0 else 0
    ...
    def analytics_performance(self, request):
        ...
        service = SupportAnalyticsService(
            support_user=support_user, days=days)
    ...
    def performance_metrics(self, request):
        ...
        days = int(request.query_params.get('days', 30))
        service = SupportPerformanceService(
            support_user=request.user, days=days)
    ...
    def sla_dashboard(self, request):
        ...
        breaches_serializer = OrderDisputeSLASerializer(
            recent_breaches[:10], many=True)
