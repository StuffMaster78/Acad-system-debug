...
def reconcile_stale_payment_intents_task(
    ...
    logger.info(
        "Payment reconciliation: checked=%s changed=%s unchanged=%s errors=%s",
        result.get("checked"),
        result.get("changed"),
...
def reconcile_pending_refunds_task(
    ...
    """
    result = PaymentReconciliationService.reconcile_pending_refunds(limit=limit)

    logger.info(
        "Refund reconciliation: checked=%s changed=%s unchanged=%s errors=%s",
        result.get("checked"),
        result.get("changed"),
        result.get("unchanged"),
        result.get("errors"),
...
def reconcile_payment_intent_by_id_task(
    ...
    logger.info(
        "On-demand reconciliation completed for intent=%s changed=%s",
        payment_intent_id,
        result.get("changed"),
    )
