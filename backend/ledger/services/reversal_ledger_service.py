...
class ReversalLedgerService:
    """
    Creates reversing journal entries for posted entries.
    """
    ...
    def reverse_entry(
        *,
        ...
        if journal_entry.status != JournalEntryStatus.POSTED:
            raise LedgerReversalError("Only posted journal entries can be reversed.")

        if JournalEntry.objects.filter(reversal_of=journal_entry).exists():
            raise LedgerReversalError("This journal entry has already been reversed.")

        reverse_lines: list[JournalLineInput] = []

        entry_lines = JournalLine.objects.select_related(
            "ledger_account"
        ).filter(journal_entry=journal_entry)
        ...
        for line in entry_lines:
            ...
            reverse_lines.append(
                JournalLineInput(
                    ...
                    entry_side=reversed_side,
                    amount=line.amount,
                    description=f"Reversal of {journal_entry.entry_number}",
                    user=line.user,
        ...
        reversal_entry = JournalPostingService.post_entry(
            ...
            description=reason or f"Reversal of {journal_entry.entry_number}",
            ...
            payment_intent_reference=journal_entry.payment_intent_reference,
