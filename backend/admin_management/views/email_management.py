...
from mass_emails.permissions import CanManageMassEmails
from mass_emails.selectors import MassEmailCampaignSelector
...
class MassEmailManagementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = MassEmailCampaignSelector.visible_to(
            self.request.user,
...
class EmailDigestManagementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = NotificationDigest.objects.select_related(
            'user',
            'website',
        )
    ...
    def configs(self, request):
        ...
        from notifications_system.registry import (
            get_digest_config,
            list_all_event_keys,
        )
...
class BroadcastMessageManagementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        queryset = BroadcastNotification.objects.select_related(
            'website',
            'created_by',
        )
    ...
    def stats(self, request, pk=None):
        ...
        acknowledged = BroadcastAcknowledgement.objects.filter(
            broadcast=broadcast,
        ).count()
        acknowledgement_rate = 0
        if total_recipients > 0:
            acknowledgement_rate = acknowledged / total_recipients * 100
