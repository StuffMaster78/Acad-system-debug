...
class RefundLogAdmin(admin.ModelAdmin):
    list_display = ('id', 'order', 'amount', 'refund', 'client', 'source', 'status', 'created_at')
    list_filter = ('status', 'source')
    search_fields = ('order__id', 'refund__id', 'client__username')
    readonly_fields = ('created_at', 'processed_by', 'action', 'metadata', 'client', 'refund', 'order', 'amount', 'source', 'status')
    ordering = ('-created_at',)
...
class RefundReceiptAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('generated_at', 'refund', 'amount', 'order_payment', 'client', 'processed_by', 'reason')
    list_filter = ('refund__status', 'refund__refund_method')
    raw_id_fields = ('refund', 'order_payment', 'client', 'processed_by')
    fieldsets = (
        (None, {
            'fields': ('refund', 'reference_code', 'amount', 'order_payment', 'client', 'processed_by', 'reason')
        }),
        ('Timestamps', {
            'fields': ('generated_at',)
        }),
