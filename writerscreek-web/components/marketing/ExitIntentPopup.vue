        ...
        <div class="animate-popup relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-950/8">
          ...
          <div class="p-6" :class="cfg.image_url ? '' : 'pt-8'">
            <p class="pr-6 text-xl font-bold leading-snug text-slate-900">
              {{ cfg.title || 'Start earning today — apply in 5 minutes.' }}
            </p>
            <p class="mt-2 text-sm leading-relaxed text-slate-500">
              {{ cfg.body || 'Join 2,000+ credentialed writers on Writers Creek. Competitive rates, flexible schedule, reliable bi-weekly payouts.' }}
            </p>
            ...
            <div class="mt-5 flex flex-col gap-2 sm:flex-row-reverse sm:justify-start">
              ...
              <a
                ...
                @click="popup.dismiss()"
              >{{ cfg.primary_cta_label || 'Apply now' }}</a>
              <NuxtLink
                v-else
                :to="cfg.primary_cta_url || '/apply'"
                class="inline-flex items-center justify-center rounded-xl bg-brand-600 px-6 py-2.5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-brand-700"
                @click="popup.dismiss()"
              >{{ cfg.primary_cta_label || 'Apply now' }}</NuxtLink>
