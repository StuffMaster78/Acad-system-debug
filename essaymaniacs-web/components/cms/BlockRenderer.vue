...
// ── Flat-link rewriter ────────────────────────────────────────────────────────
// Safety net for scraped Wagtail content where internal links were stored as
// flat paths (e.g. /essays) before the site moved to /services/<slug>.
// The backend `fix_cms_links` command is the primary fix; this handles any
// remaining cases client-side.
...
const _prodHostname = 'essaymaniacs.com'
...
function rewriteLinks(html: string): string {
  ...
  // Step 0: Collapse same-site absolute URLs to relative paths. Matches both
  // current hostname and the production domain so dev links don't leak to live site.
  ...
  // Step 1: Rewrite single-segment relative paths to flat canonical URLs.
  ...
  // Step 2: Mark remaining absolute (genuinely external) links.
  ...
  <template v-for="(block, i) in enrichedBlocks" :key="i">
      ...
      <div class="overflow-hidden rounded-2xl shadow-md ring-1 ring-slate-900/5">
          ...
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p class="font-semibold text-white text-sm leading-snug">
                {{ asStr(asObj(block.value).title) || 'Academic Writing Sample' }}
              </p>
            </div>
            <div class="flex flex-wrap gap-1.5">
              <span class="rounded-full bg-white/15 px-3 py-1 text-[10px] font-bold text-white/90 backdrop-blur-sm">
        ...
        <div class="flex items-center justify-between gap-4 border-t border-slate-200 bg-white px-6 py-4">
          ...
          <div class="flex items-center gap-3">
            <a v-if="asObj(asObj(block.value).attachment).slug"
              :href="`/resources/${asStr(asObj(asObj(block.value).attachment).slug)}`"
              ...
              {{ asStr(asObj(block.value).download_cta) || 'Full Sample' }}
            </a>
            <a :href="app.order"
              class="inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-4 py-1.5 text-xs font-bold text-white hover:bg-brand-700 transition-colors">
              Order like this
            </a>
          </div>
        </div>
    ...
    <div v-else-if="block.type === 'sources'" class="my-10 not-prose">
      <div class="rounded-2xl border border-slate-200 overflow-hidden">
        <div class="flex items-center gap-3 bg-slate-50 border-b border-slate-200 px-5 py-3.5">
          ...
          <span class="ml-auto rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-bold text-slate-600">
            {{ asArr(asObj(block.value).sources).length }}
          </span>
        </div>
        <ol class="divide-y divide-slate-100">
          <li
            ...
            :key="j"
            class="flex gap-4 px-5 py-4 hover:bg-slate-50 transition-colors"
          >
            ...
            <div class="min-w-0 flex-1">
              <a :href="asStr(asObj(src).url)" target="_blank" rel="noopener noreferrer"
                class="text-sm font-semibold text-ink hover:text-brand-600 transition-colors hover:underline line-clamp-2 flex items-start gap-1">
                {{ asStr(asObj(src).title) }}
                <ExternalLink class="size-3 shrink-0 mt-0.5 text-slate-400" />
              </a>
    ...
    <div v-else-if="block.type === 'attachment'" class="my-6 not-prose">
      ...
      <div v-if="asStr(asObj(block.value).display_style) === 'hero'"
        class="overflow-hidden rounded-2xl bg-gradient-to-br from-brand-900 to-brand-700 shadow-lg">
        <div class="px-7 py-7 sm:flex sm:items-center sm:justify-between sm:gap-6">
          <div class="flex items-start gap-4">
            <div class="flex size-14 shrink-0 items-center justify-center rounded-xl bg-white/15 text-2xl text-white">
              <span v-if="asStr(asObj(asObj(block.value).attachment).file_format).toUpperCase() === 'PDF'">📄</span>
              <span v-else-if="['DOCX','DOC'].includes(asStr(asObj(asObj(block.value).attachment).file_format).toUpperCase())">📝</span>
              <span v-else>📁</span>
            </div>
            <div class="min-w-0">
              ...
              <p class="font-bold text-white text-base leading-snug">
                {{ asStr(asObj(block.value).custom_title) || asStr(asObj(asObj(block.value).attachment).title) }}
              </p>
              <p v-if="asStr(asObj(asObj(block.value).attachment).description)" class="mt-1 text-sm text-brand-200 leading-relaxed line-clamp-2">
                {{ asStr(asObj(asObj(block.value).attachment).description) }}
              </p>
            </div>
          </div>
          <a :href="asStr(asObj(asObj(block.value).attachment).slug) ? `/resources/${asStr(asObj(asObj(block.value).attachment).slug)}` : '#'"
            ...
            {{ asStr(asObj(block.value).custom_cta_text) || 'Download Now' }}
          </a>
        </div>
      </div>
      <!-- Card style (default) -->
      <div v-else-if="!asStr(asObj(block.value).display_style) || asStr(asObj(block.value).display_style) === 'card'"
        ...
        <div class="flex size-12 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-xl text-brand-600">
          <span v-if="asStr(asObj(asObj(block.value).attachment).file_format).toUpperCase() === 'PDF'">📄</span>
          <span v-else>📁</span>
        </div>
        <div class="min-w-0 flex-1">
          ...
          <p v-if="asStr(asObj(asObj(block.value).attachment).description)" class="mt-0.5 text-xs text-slate-500 line-clamp-2">
            {{ asStr(asObj(asObj(block.value).attachment).description) }}
          </p>
          <div class="mt-2 flex flex-wrap items-center gap-2">
            <span v-if="asStr(asObj(asObj(block.value).attachment).file_format)"
              class="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase text-slate-500">
              {{ asStr(asObj(asObj(block.value).attachment).file_format) }}
            </span>
            <a :href="asStr(asObj(asObj(block.value).attachment).slug) ? `/resources/${asStr(asObj(asObj(block.value).attachment).slug)}` : '#'"
      ...
      <div v-else-if="asStr(asObj(block.value).display_style) === 'list'"
        ...
        <span class="flex-1 min-w-0 text-sm font-medium text-ink truncate">
          {{ asStr(asObj(block.value).custom_title) || asStr(asObj(asObj(block.value).attachment).title) }}
        </span>
        <a :href="asStr(asObj(asObj(block.value).attachment).slug) ? `/resources/${asStr(asObj(asObj(block.value).attachment).slug)}` : '#'"
          class="shrink-0 inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-brand-700 transition-colors">
          <Download class="size-3.5" />
          {{ asStr(asObj(block.value).custom_cta_text) || 'Download' }}
        </a>
      </div>
      <!-- Button style -->
      <div v-else-if="asStr(asObj(block.value).display_style) === 'button'">
        <a :href="asStr(asObj(asObj(block.value).attachment).slug) ? `/resources/${asStr(asObj(asObj(block.value).attachment).slug)}` : '#'"
          class="inline-flex items-center gap-2 rounded-xl border border-brand-200 bg-brand-50 px-5 py-2.5 text-sm font-semibold text-brand-700 hover:bg-brand-100 transition-colors">
          <Download class="size-4" />
          {{ asStr(asObj(block.value).custom_cta_text) || asStr(asObj(block.value).custom_title) || 'Download File' }}
        </a>
      </div>
    </div>
    ...
    <div v-else-if="block.type === 'related_posts'" class="my-8 not-prose">
      <p v-if="asStr(asObj(block.value).heading)" class="mb-4 text-xs font-bold uppercase tracking-widest text-slate-400">
        {{ asStr(asObj(block.value).heading) }}
      </p>
      <div class="grid gap-3 sm:grid-cols-2">
        <a
          v-for="(post, j) in asArr(asObj(block.value).posts)" :key="j"
          :href="pageHref(asObj(asObj(post).meta))"
        ...
        >
          <div class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-600 font-bold text-sm">
            {{ j + 1 }}
          </div>
          <div class="min-w-0">
            <p class="text-sm font-semibold text-ink group-hover:text-brand-700 transition-colors line-clamp-2">
              {{ asStr(asObj(post).title) }}
            </p>
          </div>
          <ArrowRight class="size-4 shrink-0 text-slate-300 group-hover:text-brand-500 group-hover:translate-x-0.5 transition-all ml-auto" />
        </a>
      </div>
    </div>
    ...
    <div v-else-if="block.type === 'rich_cta'" class="my-10 not-prose">
      ...
      <div v-if="!asStr(asObj(block.value).style) || asStr(asObj(block.value).style) === 'gradient'"
        ...
        <div class="absolute top-0 right-0 w-72 h-72 rounded-full bg-white/5 -translate-y-1/2 translate-x-1/3 blur-3xl" />
        <div class="relative px-7 py-7 sm:flex sm:items-center sm:justify-between sm:gap-6">
          <div class="flex-1 min-w-0">
            <p v-if="asStr(asObj(block.value).eyebrow)" class="mb-1.5 text-[10px] font-bold uppercase tracking-widest text-brand-300">
              {{ asStr(asObj(block.value).eyebrow) }}
            </p>
            ...
            <ul v-if="asArr(asObj(block.value).trust_items).length" class="mt-3 flex flex-wrap gap-x-4 gap-y-1">
              <li v-for="(item, j) in asArr(asObj(block.value).trust_items)" :key="j"
                class="flex items-center gap-1.5 text-xs text-brand-300">
          ...
          <div class="mt-5 flex flex-col gap-2 sm:mt-0 sm:shrink-0">
            <a :href="asStr(asObj(block.value).cta_url) || app.order"
              class="flex items-center justify-center gap-2 rounded-xl bg-white px-6 py-3 text-sm font-bold text-brand-800 shadow transition-colors hover:bg-brand-50">
              {{ asStr(asObj(block.value).cta_text) || 'Order Now' }}
              <ArrowRight class="size-4" />
            </a>
            <a v-if="asStr(asObj(block.value).secondary_cta_text)"
              :href="asStr(asObj(block.value).secondary_cta_url) || '#'"
              class="text-center text-xs font-semibold text-brand-300 hover:text-white transition-colors hover:underline">
              {{ asStr(asObj(block.value).secondary_cta_text) }}
            </a>
          </div>
        </div>
      </div>
      ...
      <!-- Split image style -->
      <div v-else-if="asStr(asObj(block.value).style) === 'split_image'"
        class="overflow-hidden rounded-2xl border border-slate-200 shadow-sm sm:flex">
        ...
        <div class="flex flex-1 flex-col justify-center bg-brand-50 px-6 py-6">
          <p v-if="asStr(asObj(block.value).eyebrow)" class="mb-1 text-[10px] font-bold uppercase tracking-widest text-brand-600">
            {{ asStr(asObj(block.value).eyebrow) }}
          </p>
          ...
          <ul v-if="asArr(asObj(block.value).trust_items).length" class="mt-3 space-y-1">
            <li v-for="(item, j) in asArr(asObj(block.value).trust_items)" :key="j"
              ...
              <CheckCircle2 class="size-3.5 text-brand-600 shrink-0" />
              {{ asStr(item) }}
            </li>
          </ul>
          <a :href="asStr(asObj(block.value).cta_url) || app.order"
            class="mt-4 inline-flex items-center gap-2 self-start rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-brand-700 transition-colors">
            {{ asStr(asObj(block.value).cta_text) || 'Order Now' }}
            <ArrowRight class="size-4" />
          </a>
        </div>
      ...
      <!-- Light style -->
      <div v-else-if="asStr(asObj(block.value).style) === 'light'"
        class="rounded-2xl border border-brand-200 bg-brand-50 px-6 py-6">
        <p v-if="asStr(asObj(block.value).eyebrow)" class="mb-1 text-[10px] font-bold uppercase tracking-widest text-brand-600">
          {{ asStr(asObj(block.value).eyebrow) }}
        </p>
        <div class="sm:flex sm:items-center sm:justify-between sm:gap-6">
          <div class="flex-1 min-w-0">
            ...
            <ul v-if="asArr(asObj(block.value).trust_items).length" class="mt-2 flex flex-wrap gap-x-4 gap-y-1">
              <li v-for="(item, j) in asArr(asObj(block.value).trust_items)" :key="j"
                class="flex items-center gap-1.5 text-xs text-slate-600">
                <CheckCircle2 class="size-3.5 text-brand-600 shrink-0" />
                {{ asStr(item) }}
          ...
          <a :href="asStr(asObj(block.value).cta_url) || app.order"
            class="mt-4 flex items-center gap-2 rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-brand-700 transition-colors sm:mt-0 sm:shrink-0">
            {{ asStr(asObj(block.value).cta_text) || 'Order Now' }}
          </a>
        </div>
      </div>
      ...
      <!-- Urgent style -->
      <div v-else-if="asStr(asObj(block.value).style) === 'urgent'"
        class="overflow-hidden rounded-2xl border border-amber-300 bg-amber-50">
        <div class="bg-amber-500 px-5 py-1.5">
          <p class="text-[10px] font-bold uppercase tracking-widest text-white">
            {{ asStr(asObj(block.value).eyebrow) || '⏰ Limited Time' }}
          </p>
        </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <ul v-if="asArr(asObj(block.value).trust_items).length" class="mt-2 flex flex-wrap gap-x-4 gap-y-1">
              <li v-for="(item, j) in asArr(asObj(block.value).trust_items)" :key="j"
                ...
                <CheckCircle2 class="size-3.5 text-amber-600 shrink-0" />
                {{ asStr(item) }}
              </li>
            </ul>
          </div>
          <a :href="asStr(asObj(block.value).cta_url) || app.order"
            class="mt-4 flex items-center gap-2 rounded-xl bg-amber-600 px-6 py-2.5 text-sm font-bold text-white hover:bg-amber-700 transition-colors sm:mt-0 sm:shrink-0">
            {{ asStr(asObj(block.value).cta_text) || 'Claim Now' }}
            <ArrowRight class="size-4" />
          </a>
        </div>
      </div>
      ...
      <div v-else-if="asStr(asObj(block.value).style) === 'minimal'"
        ...
        <a :href="asStr(asObj(block.value).cta_url) || app.order"
          class="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-xs font-bold text-white hover:bg-brand-700 transition-colors">
          {{ asStr(asObj(block.value).cta_text) || 'Order Now' }}
        </a>
      </div>
