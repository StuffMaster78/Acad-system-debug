...
// Treat unselected spacing as double for word-count display
...
<template>
  <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
    ...
    <div class="border-b border-slate-100 bg-slate-50 px-5 py-4">
      <p class="text-xs font-bold uppercase tracking-widest text-brand-600">Instant price estimate</p>
        ...
        <div v-if="serviceType === 'writing'" class="rounded-lg border border-emerald-100 bg-emerald-50 p-4">
          ...
          <ul class="grid grid-cols-2 gap-x-3 gap-y-1.5">
            <li
              ...
              :key="item" class="flex items-center gap-1.5 text-[11px] font-medium text-emerald-800"
            >
      ...
      <div class="rounded-xl border border-brand-100 bg-brand-50 p-4">
        <div class="flex items-end justify-between gap-2">
          <div>
            ...
            <p class="mt-0.5 text-3xl font-extrabold tabular-nums leading-none text-brand-800">
        ...
        <div class="mt-3 flex flex-wrap gap-x-3 gap-y-1 border-t border-brand-100 pt-3 text-[11px] text-slate-500">
          <span>✓ Pay after approval</span>
          <span>✓ Free revisions</span>
          <span>✓ Human-written</span>
        </div>
        <a :href="orderUrl"
          class="mt-3 flex h-11 w-full items-center justify-center rounded-xl bg-brand-600 text-sm font-bold text-white shadow-sm transition-colors hover:bg-brand-700">
          Start my order — {{ selectedLevel?.label }}
        </a>
        <p class="mt-2 text-center text-[11px] text-slate-400">Price confirmed at checkout · no card needed</p>
      </div>

      <p class="text-center text-xs text-slate-400">
        Complex scope?
        <NuxtLink to="/quote" class="font-semibold text-brand-600 transition-colors hover:text-brand-700">Get a custom quote →</NuxtLink>
      </p>
...
.sel {
  @apply h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-800
