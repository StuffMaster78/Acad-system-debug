  ...
  <div class="border-t border-slate-800 py-10">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p class="text-sm font-semibold text-white">Essay writing tips in your inbox</p>
          <p class="mt-0.5 text-xs text-slate-400">Weekly writing guides, citation tips, and exclusive discounts. Unsubscribe any time.</p>
        </div>
        <form v-if="!success" class="flex w-full max-w-md gap-2" @submit.prevent="submit">
          ...
          <button
            type="submit"
            :disabled="busy"
            class="inline-flex shrink-0 items-center rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-brand-700 disabled:opacity-60"
          >
            {{ busy ? '…' : 'Subscribe' }}
          </button>
        </form>
