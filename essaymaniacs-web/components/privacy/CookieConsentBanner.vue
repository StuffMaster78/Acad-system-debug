  ...
  <div v-if="show" class="fixed inset-x-0 bottom-0 z-50 px-4 pb-4 sm:px-6">
    <div class="mx-auto max-w-5xl rounded-lg border border-slate-200 bg-white p-4 shadow-2xl">
      <div class="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-start">
        <div>
          <p class="text-sm font-bold text-slate-950">Cookie preferences</p>
          ...
          <button class="mt-2 text-sm font-semibold text-brand-700 underline underline-offset-4" type="button" @click="consent.openSettings()">
            Manage settings
          </button>
        </div>
        <div class="flex flex-wrap gap-2 lg:justify-end">
          ...
          <button class="rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700" type="button" @click="consent.acceptAll()">
            Accept all
          </button>
        </div>
      </div>
      ...
      <div v-if="consent.settingsOpen.value" class="mt-4 border-t border-slate-200 pt-4">
        ...
        <div class="mt-4 flex justify-end gap-2">
          ...
          <button class="rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700" type="button" @click="consent.save(draft, 'settings')">
            Save settings
          </button>
        </div>
      </div>
