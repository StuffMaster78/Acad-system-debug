...
useHead({
  ...
  script: [{
    ...
    innerHTML: JSON.stringify({
      ...
      '@type': 'FAQPage',
      mainEntity: [
        { '@type': 'Question', name: 'Is EssayManiacs legit?', acceptedAnswer: { '@type': 'Answer', text: "Yes. EssayManiacs is a legitimate academic assistance service. Every writer holds at minimum a Master's degree verified against the issuing institution. We have delivered 20,000+ papers with a 4.8/5 average rating." } },
...
const sections = [
  {
    ...
    emoji: '🏢',
    items: [
      { q: 'Is EssayManiacs legit?', a: "Yes. EssayManiacs is a legitimate academic assistance platform. Every writer holds at minimum a Master's degree that we verify before they can accept orders. We have delivered 20,000+ papers with a 4.8/5 average rating." },
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'What if I am not satisfied with my paper?', a: "Request a revision first — most quality concerns are resolved in one revision round. If after revision you are still dissatisfied within the terms of our grade guarantee, email support@essaymaniacs.com with your order number and details." },
    ],
  },
  {
    ...
    items: [
      ...
      { q: 'What does my statement show?', a: "Your bank statement will show the payment processor name, not EssayManiacs — for your privacy." },
    ],
  },
]
</script>
      ...
      <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        <div class="space-y-12">
          <div v-for="section in sections" :key="section.heading">
            ...
            <div class="space-y-2">
              <details v-for="item in section.items" :key="item.q" class="group overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                <summary class="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-sm font-semibold text-slate-800 hover:bg-white transition-colors">
                  {{ item.q }}
                  <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-white text-slate-400 shadow-sm transition-transform group-open:rotate-45">+</span>
                </summary>
                <p class="border-t border-slate-100 bg-white px-5 py-4 text-sm leading-relaxed text-slate-600">{{ item.a }}</p>
              </details>
            </div>
          </div>
        </div>
