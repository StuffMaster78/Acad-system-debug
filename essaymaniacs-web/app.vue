...
useHead({
  script: computed(() => portal.schemaOrgName ? [
    { type: 'application/ld+json', innerHTML: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Organization', name: portal.schemaOrgName, url: `https://${portal.ctx.website?.domain ?? 'essaymaniacs.com'}`, ...(portal.schemaOrgLogo ? { logo: portal.schemaOrgLogo } : {}) }) },
    { type: 'application/ld+json', innerHTML: JSON.stringify({ '@context': 'https://schema.org', '@type': 'WebSite', name: portal.schemaOrgName, url: `https://${portal.ctx.website?.domain ?? 'essaymaniacs.com'}` }) },
  ] : []),
})
