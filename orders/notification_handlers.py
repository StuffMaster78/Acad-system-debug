...
@register_handler("order_assigned")
class OrderAssignedHandler(BaseNotificationHandler):
...
@register_handler("order_paid")
class OrderPaidHandler(BaseNotificationHandler):
    def handle(self, *, user, event, payload=None, **kwargs):
        from notifications_system.services.core import NotificationService
        NotificationService.send_notification(
            user=user,
