...
onMounted(() => { window.location.replace(app.order) })
</script>
...
<template>
  <div class="pt-16 min-h-screen flex items-center justify-center bg-mist">
    <div class="text-center space-y-3 px-4">
      ...
      <p class="text-sm text-graphite">Taking you to the order form…</p>
      <a :href="app.order" class="text-sm font-semibold text-gc-600 hover:text-gc-700 transition-colors">Click here if not redirected</a>
    </div>
  </div>
</template>
