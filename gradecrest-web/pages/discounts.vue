...
useSeoMeta({
  title: 'Academic Writing Discounts & Promo Codes | GradeCrest',
  description: 'Save on expert academic writing. First-order discounts, loyalty rewards, referral credits, and seasonal promos — see current deals and how to earn more at GradeCrest.',
  ogImageWidth:  1200,
  ogImageHeight: 630,
...
useHead({
  link: [{ rel: 'canonical', href: 'https://gradecrest.com/discounts' }],
  script: [{
    ...
    innerHTML: JSON.stringify({
      ...
      mainEntity: [
        {
          '@type': 'Question',
          name: 'Does GradeCrest offer first-order discounts?',
          acceptedAnswer: { '@type': 'Answer', text: 'Yes. New clients receive a first-order discount automatically at checkout — no code needed when placing your first order.' },
        },
        {
          '@type': 'Question',
          name: 'How do loyalty rewards work at GradeCrest?',
        ...
        {
          '@type': 'Question',
          name: 'Is there a referral discount at GradeCrest?',
...
<template>
  <div>
    ...
    <Transition name="slide-down">
      <div v-if="visible && promo" class="relative bg-gc-700 px-4 py-3 text-center text-sm font-medium text-white">
    ...
    <!-- Hero -->
    <section class="bg-gradient-to-br from-forest-950 via-forest-900 to-gc-900 px-4 py-20 text-center text-white">
      <div class="mx-auto max-w-3xl">
        <p class="mb-3 text-xs font-bold uppercase tracking-widest text-gc-300">Savings &amp; Promotions</p>
        <h1 class="font-serif text-4xl font-bold leading-tight sm:text-5xl">Academic writing discounts<br>you actually earn</h1>
        <p class="mt-5 text-lg text-forest-200 leading-relaxed">First-order deals, loyalty points, referral credits, and seasonal promos — every way to save on expert academic writing, all in one place.</p>
        <div class="mt-8 flex flex-wrap justify-center gap-3">
          <NuxtLink to="/order" class="rounded-xl bg-white px-6 py-3 text-sm font-bold text-forest-900 shadow transition hover:bg-gc-50">Place an order</NuxtLink>
          <NuxtLink to="/auth/login" class="rounded-xl border border-white/30 px-6 py-3 text-sm font-semibold text-white transition hover:bg-white/10">Sign in to redeem</NuxtLink>
        </div>
      </div>
    </section>
    ...
    <section class="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="w in ways" :key="w.title" class="flex flex-col rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
          ...
          <NuxtLink :to="w.href" class="text-sm font-semibold text-gc-700 hover:text-gc-800">{{ w.cta }} →</NuxtLink>
        </div>
      </div>
    </section>
    ...
    <section class="border-y border-slate-100 bg-slate-50 px-4 py-14">
      <div class="mx-auto max-w-4xl text-center">
        ...
        <NuxtLink to="/order" class="mt-10 inline-flex items-center gap-2 rounded-xl bg-gc-700 px-6 py-3 text-sm font-bold text-white shadow transition hover:bg-gc-800">Place an order →</NuxtLink>
      </div>
    </section>
      ...
      <div class="divide-y divide-slate-100 rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
        <details v-for="faq in faqs" :key="faq.q" class="group px-5 py-4">
          <summary class="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-semibold text-slate-900">
            {{ faq.q }}
            <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-gc-100 text-gc-600 transition-transform group-open:rotate-45 text-xs font-bold">+</span>
          </summary>
    ...
    <!-- CTA -->
    <section class="bg-forest-950 px-4 py-16 text-center text-white">
      <div class="mx-auto max-w-xl">
        <h2 class="font-serif text-2xl font-bold">Ready to place your order?</h2>
        <p class="mt-3 text-forest-300 text-sm">Expert writers. Grade guarantee. Delivered on time.</p>
        <div class="mt-8 flex flex-wrap justify-center gap-3">
          <NuxtLink to="/order" class="rounded-xl bg-white px-6 py-3 text-sm font-bold text-forest-900 shadow transition hover:bg-gc-50">Start my order</NuxtLink>
