    ...
    <div v-if="popup.open.value && cfg" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/55 px-4">
      <div class="relative w-full max-w-md rounded-lg bg-white p-6 shadow-2xl">
        ...
        <div class="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
          <button v-if="cfg.secondary_cta_label" class="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50" type="button" @click="popup.dismiss()">
            {{ cfg.secondary_cta_label }}
          </button>
          <NuxtLink class="rounded-md bg-slate-950 px-4 py-2 text-center text-sm font-semibold text-white hover:bg-slate-800" :to="cfg.primary_cta_url || '/quote'" @click="popup.dismiss()">
            {{ cfg.primary_cta_label || 'Continue' }}
          </NuxtLink>
        </div>
      </div>
    </div>
