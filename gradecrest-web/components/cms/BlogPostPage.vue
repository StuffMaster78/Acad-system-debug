...
const staticPost = article.value ? null : getBySlug(props.slug)

// Connected articles
const { data: connectedPosts } = await useAsyncData<ArticleDetail[]>(
  `connected-${props.slug}`,
  async () => {
    try {
      const res = await $fetch<{ items: ArticleDetail[] }>(
        `${wagtailBase}/api/v2/pages/`,
...
if (article.value) {
  ...
  useSeoBase(`https://gradecrest.com/${props.slug}`)
  useBreadcrumbs([
    { name: 'Home', url: 'https://gradecrest.com/' },
    { name: 'Blog', url: 'https://gradecrest.com/blog' },
    { name: article.value.title, url: `https://gradecrest.com/${props.slug}` },
  ])
...
// ── Engagement ────────────────────────────────────────────────────────────
const { stats: engStats, myReact, ready: engReady, react, reactionCount, fmtCount } = useEngagement(props.slug)
...
// Sharing
const pageUrl = computed(() => `https://gradecrest.com/${props.slug}`)
