...
useHead({
  ...
  script: [
    { type: 'application/ld+json', innerHTML: JSON.stringify({ '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Home', item: `${siteUrl}/` }, { '@type': 'ListItem', position: 2, name: 'Services', item: `${siteUrl}/services` }, { '@type': 'ListItem', position: 3, name: 'iHuman Virtual Patient Help', item: canonicalUrl }] }) },
    { type: 'application/ld+json', innerHTML: JSON.stringify({ '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: [
    ...
    <div class="bg-white">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div class="lg:grid lg:grid-cols-3 lg:gap-12 xl:gap-16">
          ...
          <!-- Left 2/3 — scrollable content (Wagtail or static fallback) -->
          <article class="lg:col-span-2 space-y-12 min-w-0" aria-label="iHuman service details">

            <!-- CMS body when a Wagtail ServicePage is published -->
            <template v-if="cmsPage?.body?.length">
              <BlockRenderer :blocks="cmsPage.body" />
            </template>

            <!-- Static content shown until Wagtail content is published -->
            <template v-else>
              ...
              <section aria-labelledby="how-heading">
                ...
                <ol class="relative border-l border-brand-200 ml-3 space-y-0">
                  <li v-for="(step_item, idx) in [
                    ...
                    <p class="font-semibold text-slate-900 text-sm">{{ step_item.title }}</p>
                    <p class="mt-1 text-sm text-slate-500 leading-relaxed">{{ step_item.desc }}</p>
                  </li>
                </ol>
              </section>
