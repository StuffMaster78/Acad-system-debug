...
const allTocItems = computed(() => {
  if (cmsArticle.value) {
    // Prefer structured heading blocks
    if (cmsToc.value.length >= 3) return cmsToc.value
    // Fall back: parse h2/h3 directly from paragraph HTML.
    // Uses slugifyHeading — must match injectHeadingIds in BlockRenderer.vue.
