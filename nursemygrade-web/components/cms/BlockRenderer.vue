...
function sphereClass(style: string): string {
  return SPHERE_CLASS[style] ?? SPHERE_CLASS.sphere_solid
}
...
function hlColor(color: string, key: string): string {
  return (HL_COLORS[color] ?? HL_COLORS.brand)[key] ?? ''
}
...
function chipColor(color: string): string {
  return CHIP_COLOR[color] ?? CHIP_COLOR.brand
}
