...
<template>
  <div>
    <template v-for="block in blocks" :key="block.id">
      ...
      <!-- heading — break-after-avoid keeps heading with the paragraph that follows -->
      ...
      <!-- paragraph — no break-inside-avoid: lets CSS columns flow through prose naturally -->
      ...
      <template v-else-if="block.type === 'image'">
        ...
        >
          <figure>
            ...
            />
            <figcaption v-if="(block.value as ImageVal).caption" class="mt-2 text-center text-xs italic text-slate-400">
