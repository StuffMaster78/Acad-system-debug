...
<template>
  <div class="border-t border-brand-800 py-14">
    <div class="mx-auto max-w-lg px-4 text-center">
      ...
      <div class="mx-auto mb-4 flex size-11 items-center justify-center rounded-full bg-brand-500/20">
        <svg class="size-5 text-brand-300" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"/>
        </svg>
      </div>

      <h2 class="text-xl font-bold text-white sm:text-2xl">Clinical study tips for nursing students</h2>
      <p class="mt-1.5 text-sm text-brand-300">Care plans, SOAP notes, and APA guides — in your inbox every week.</p>
      ...
      <form v-if="!success" class="mt-6 space-y-3" @submit.prevent="submit" novalidate>
        <div class="flex flex-col gap-2 sm:flex-row">
          <input
            ...
            autocomplete="email"
            class="min-w-0 flex-1 rounded-xl border px-4 py-3 text-sm text-white placeholder:text-brand-600 focus:outline-none focus:ring-2 transition-colors"
            :class="showInvalid
              ? 'border-red-500/60 bg-red-950/20 focus:ring-red-500/20'
              : 'border-brand-700 bg-brand-800/60 focus:border-brand-400 focus:ring-brand-400/20'"
            @blur="touched = true"
            @input="touched = false"
          />
          <button
            type="submit"
            :disabled="busy"
            class="shrink-0 rounded-xl bg-brand-500 px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-brand-400 disabled:opacity-60"
        ...
        <p class="text-xs text-brand-500">🔒 No spam · Unsubscribe any time</p>
        <p class="text-xs text-brand-600 leading-relaxed">
          ...
          <NuxtLink to="/privacy" class="underline underline-offset-2 hover:text-brand-400 transition-colors">Privacy Policy</NuxtLink>.
        </p>
      </form>
      ...
      <div v-else class="mt-6 flex flex-col items-center gap-3">
        ...
        <p class="text-xs text-brand-400">Check your inbox for a confirmation email.</p>
      </div>
