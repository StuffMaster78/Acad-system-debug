...
from utils.jwt import encode_magic_link_token, decode_magic_link_token, get_tokens_for_user
...
class MagicLinkRequestView(APIView):
    """
    Accepts an email, sends a magic link to that email if user exists.
    """
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"detail": "If that email exists, a magic link was sent."}, status=status.HTTP_200_OK)

        token = encode_magic_link_token(user)

        current_site = get_current_site(request)
        domain = current_site.domain
        path = reverse("magic-link-login")  # make sure your URLconf has this name
        magic_link_url = f"https://{domain}{path}?token={token}"

        subject = "Your Magic Login Link"
        message = (
            f"Hi {user.get_full_name() or user.username},\n\n"
            f"Use the link below to login. This link expires in 15 minutes.\n\n"
            f"{magic_link_url}\n\n"
...
class MagicLinkLoginView(APIView):
    """
    Accepts magic link token from query params, returns JWT tokens if valid.
    """

    def get(self, request):
        token = request.query_params.get("token")
        if not token:
            return Response({"detail": "Token query param required."}, status=status.HTTP_400_BAD_REQUEST)
