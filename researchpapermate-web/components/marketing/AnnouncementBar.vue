...
const code     = computed(() => bar.value?.code    || 'PAPER15')
const message  = computed(() => bar.value?.message || 'First order?')
const suffix   = computed(() => bar.value?.suffix  || 'for 15% off · Free plagiarism report on every research paper')
