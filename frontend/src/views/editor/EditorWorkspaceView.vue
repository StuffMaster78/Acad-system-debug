...
const activeTab = computed(() => {
  if (route.path.includes("/qa")) return "qa";
  if (route.path.includes("/workload")) return "workload";
...
const activityRows = computed(() => workspace.activity.activity_logs ?? []);
...
<template>
  <div class="space-y-6">
      ...
      <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 class="text-lg font-semibold text-ink">
              {{ activeTab === "qa" ? "QA queue" : "Assigned reviews" }}
            </h2>
            <p class="mt-1 text-sm text-graphite">
              Review assignments, deadlines, and task actions.
            </p>
          </div>
          <button
        ...
        <div class="mt-5 overflow-hidden rounded-md border border-slate-200">
          ...
          <button
            ...
            :class="selectedTask?.id === task.id ? 'bg-slate-50' : 'bg-white'"
            type="button"
    ...
    <section v-if="activeTab === 'qa' || activeTab === 'desk'" class="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
      <div class="flex items-center justify-between gap-3">
        <div>
      ...
      <div class="mt-5 grid gap-3 lg:grid-cols-2">
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <StatusPill :label="relativeDeadline(task)" :tone="task.order_deadline && new Date(task.order_deadline) < new Date() ? 'danger' : 'neutral'" />
          </div>
          <button
    ...
    <section class="grid gap-6 xl:grid-cols-3">
      <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-panel xl:col-span-2">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-lg font-semibold text-ink">Workload and analytics</h2>
        </div>
        <div class="mt-5 grid gap-4 md:grid-cols-3">
          ...
          <div class="rounded-md border border-slate-200 p-4">
            <p class="text-sm font-medium text-graphite">Avg hours per task</p>
        ...
        <div class="mt-5 grid gap-4 md:grid-cols-2">
          <div class="rounded-md border border-slate-200 p-4">
            <h3 class="text-sm font-semibold text-ink">Status mix</h3>
            <div class="mt-3 space-y-2">
              <div
                v-for="([status, count]) in Object.entries(workspace.analytics.status_breakdown ?? {})"
                :key="status"
                class="flex items-center justify-between text-sm"
              >
                <span class="capitalize text-graphite">{{ status.replace('_', ' ') }}</span>
                <span class="font-semibold text-ink">{{ count }}</span>
              </div>
            </div>
          </div>
          <div class="rounded-md border border-slate-200 p-4">
            <h3 class="text-sm font-semibold text-ink">Assignment mix</h3>
            <div class="mt-3 space-y-2">
              <div
                v-for="([kind, count]) in Object.entries(workspace.analytics.assignment_breakdown ?? {})"
                :key="kind"
                class="flex items-center justify-between text-sm"
              >
                <span class="capitalize text-graphite">{{ kind }}</span>
                <span class="font-semibold text-ink">{{ count }}</span>
              </div>
            </div>
          </div>
      ...
      <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        <h2 class="text-lg font-semibold text-ink">Recent editor activity</h2>
