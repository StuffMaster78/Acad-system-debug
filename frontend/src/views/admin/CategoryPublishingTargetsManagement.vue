<template>
  <div class="space-y-6 p-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Category Publishing Targets Management</h1>
        <p class="mt-2 text-gray-600 dark:text-gray-400">Set and track publishing targets for blog categories</p>
      </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden border border-gray-200 dark:border-gray-700">
      ...
      <div v-else-if="!targets.length" class="text-center py-12 text-gray-500 dark:text-gray-400">
        ...
        <p class="mt-2 text-sm font-medium">No category publishing targets found</p>
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="target in targets" :key="target.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ target.category?.name || target.category_id || '—' }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-900 dark:text-gray-100">{{ formatPeriod(target.period_start, target.period_end) }}</div>
              </td>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto m-4">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
          <h3 class="text-xl font-semibold text-gray-900 dark:text-gray-100">
            {{ editingTarget ? 'Edit Category Publishing Target' : 'Create Category Publishing Target' }}
          </h3>
        </div>
        <form @submit.prevent="saveTarget" class="p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Category ID *</label>
            <input
              v-model.number="targetForm.category"
              type="number"
              required
              class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
            />
...
const loadTargets = async () => {
  ...
  try {
    ...
    const res = await blogPagesAPI.getCategoryPublishingTargets(params)
...
const saveTarget = async () => {
  ...
  try {
    ...
    if (editingTarget.value) {
      await blogPagesAPI.updateCategoryPublishingTarget(editingTarget.value.id, data)
      showSuccess('Category publishing target updated successfully')
    } else {
      await blogPagesAPI.createCategoryPublishingTarget(data)
      showSuccess('Category publishing target created successfully')
    }
...
const editTarget = (target) => {
  ...
  targetForm.value = {
    category: target.category?.id || target.category_id || null,
...
const viewTarget = (target) => {
  alert(`Category Target: ${target.category?.name || target.category_id}\nProgress: ${getProgressPercent(target)}%\nPublished: ${target.published_count || 0}/${target.target_count || 0}`)
}

const deleteTarget = async (target) => {
  const confirmed = await confirm.showDestructive(
    'Are you sure you want to delete this category publishing target?',
  ...
  try {
    await blogPagesAPI.deleteCategoryPublishingTarget(target.id)
    showSuccess('Category publishing target deleted successfully')
    await loadTargets()
  } catch (error) {
