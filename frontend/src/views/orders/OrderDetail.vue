    ...
    <template v-else-if="order">
      ...
      <div v-if="activeTab === 'messages'" class="space-y-6">
        ...
        <div v-else class="space-y-6">
          ...
          <div v-if="clientWriterThreads.length > 0" class="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-200">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h4 class="text-lg font-semibold text-gray-900">Client-Writer Communication</h4>
                  <p class="text-sm text-gray-600">Direct messages between client and writer</p>
                </div>
                <span class="ml-auto px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                  {{ clientWriterThreads.length }} conversation{{ clientWriterThreads.length !== 1 ? 's' : '' }}
                </span>
              </div>
            </div>
            <div class="divide-y divide-gray-200">
              <div
                v-for="thread in clientWriterThreads"
                :key="thread.id"
                class="p-4 hover:bg-gray-50 transition-colors cursor-pointer"
                @click="toggleThreadExpanded(thread.id)"
              >
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2">
                        ...
                        <span
                          ...
                          class="px-2 py-0.5 bg-red-100 text-red-800 rounded-full text-xs font-bold"
                        >
                          {{ thread.unread_count }} new
                        </span>
                      </div>
                      <p v-if="thread.last_message" class="text-xs text-gray-500 mt-1 truncate">
                        {{ thread.last_message.message?.substring(0, 60) || 'No message' }}...
                      </p>
                      <p class="text-xs text-gray-400 mt-1">
                        {{ formatRelativeTime(thread.last_message?.sent_at || thread.updated_at) }}
                      </p>
                    </div>
                  </div>
                  <div class="flex-shrink-0 ml-4">
                    <svg
                      :class="[
                        ...
                        expandedThreads[thread.id] ? 'transform rotate-180' : ''
                      ]"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
          ...
          <!-- Staff-Client Messages -->
          <div v-if="staffClientThreads.length > 0" class="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <div class="bg-gradient-to-r from-purple-50 to-pink-50 px-6 py-4 border-b border-gray-200">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h4 class="text-lg font-semibold text-gray-900">Support & Staff Messages</h4>
                  <p class="text-sm text-gray-600">Messages with support, admin, or staff members</p>
                </div>
                <span class="ml-auto px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-xs font-medium">
                  {{ staffClientThreads.length }} conversation{{ staffClientThreads.length !== 1 ? 's' : '' }}
                </span>
              </div>
            </div>
            <div class="divide-y divide-gray-200">
              <div
                v-for="thread in staffClientThreads"
                :key="thread.id"
                class="p-4 hover:bg-gray-50 transition-colors cursor-pointer"
                @click="toggleThreadExpanded(thread.id)"
              >
          ...
          <div v-for="thread in threads" :key="`messages-${thread.id}`">
            <div
              v-if="expandedThreads[thread.id]"
              class="bg-white rounded-lg border border-gray-200 p-6 mt-4"
            >
              <div class="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
                <div>
                  <h5 class="text-md font-semibold text-gray-900">{{ getThreadTitle(thread) }}</h5>
                  <p class="text-xs text-gray-500 mt-1">
                    {{ thread.participants?.length || 0 }} participant{{ (thread.participants?.length || 0) !== 1 ? 's' : '' }}
                  </p>
                </div>
                <button
                  @click="toggleThreadExpanded(thread.id)"
              ...
              <!-- Messages Container -->
              <div
                :ref="el => { if (el) messageContainers[thread.id] = el }"
                class="max-h-96 overflow-y-auto mb-4 space-y-3"
              >
                <div v-if="loadingMessages[thread.id]" class="flex items-center justify-center py-8">
                  <div class="animate-spin rounded-full h-6 w-6 border-b-2 border-primary-600"></div>
                </div>
                
                <div v-else-if="!threadMessages[thread.id] || threadMessages[thread.id].length === 0" class="text-center py-8 text-gray-500 text-sm">
                ...
                <div v-else class="space-y-3">
                  <div
                    ...
                    :class="[
                      ...
                      message.is_sender ? 'bg-blue-50 ml-auto max-w-[80%]' : 'bg-gray-50 mr-auto max-w-[80%]'
                    ]"
                  >
                    <div class="flex-shrink-0">
                      <div :class="[
                        'w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold',
                        message.sender_role === 'client' ? 'bg-green-100 text-green-700' :
                        message.sender_role === 'writer' ? 'bg-blue-100 text-blue-700' :
                        ['admin', 'superadmin', 'support', 'editor'].includes(message.sender_role) ? 'bg-purple-100 text-purple-700' :
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2 mb-1">
                        ...
                        <span :class="[
                          ...
                          message.sender_role === 'client' ? 'bg-green-100 text-green-800' :
                          message.sender_role === 'writer' ? 'bg-blue-100 text-blue-800' :
                          ['admin', 'superadmin', 'support', 'editor'].includes(message.sender_role) ? 'bg-purple-100 text-purple-800' :
                          'bg-gray-100 text-gray-800'
                        ]">
                          {{ message.sender_role || 'User' }}
                        </span>
                        <span class="text-xs text-gray-400">{{ formatRelativeTime(message.sent_at) }}</span>
                      </div>
                      <div class="text-sm text-gray-700 whitespace-pre-wrap">{{ message.message }}</div>
                      <div v-if="message.attachment" class="mt-2">
                        <a
                          :href="getAttachmentUrl(thread.id, message.id)"
                          target="_blank"
                          class="text-xs text-blue-600 hover:text-blue-800 flex items-center gap-1"
                        >
                          <span>📎</span>
                          <span>{{ message.attachment.filename || 'Attachment' }}</span>
                        </a>
                      </div>
                    </div>
                  </div>
              ...
              <div class="border-t border-gray-200 pt-4">
                <div v-if="threadErrors[thread.id]" class="mb-2 text-xs text-red-600 bg-red-50 p-2 rounded">
                  {{ threadErrors[thread.id] }}
                </div>
                <form
                  v-if="threadMessageForms[thread.id]"
                  @submit.prevent="sendMessageToThread(thread.id)"
                  class="space-y-3"
                >
                  <div class="flex gap-2">
                    <input
                      v-model="threadMessageForms[thread.id].message"
                      type="text"
                      ...
                      @keydown.enter.prevent="sendMessageToThread(thread.id)"
                    />
                    <button
                      type="submit"
                      :disabled="sendingMessageToThreads[thread.id] || !threadMessageForms[thread.id].message?.trim()"
                    ...
                    >
                      {{ sendingMessageToThreads[thread.id] ? 'Sending...' : 'Send' }}
                    </button>
                  </div>
                </form>
                <div v-else class="text-xs text-gray-400">Preparing message composer...</div>
              </div>
            </div>
          </div>
        </div>
